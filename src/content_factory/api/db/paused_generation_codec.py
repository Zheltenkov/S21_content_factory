...
def _hydrate_value(value: Any, events: list[CompatibilityEvent]) -> Any:
    ...
    if type_name == "builtins:bytes":
        try:
            ...
            return b""
    if type_name in {
        "content_factory.generation.agents.flow:FlowExecutionStep",
        "content_factory.generation.workflow.flow_runner:FlowExecutionStep",
    }:
        payload = dict(data)
...
def _resolve_pydantic_model(type_name: str) -> type[BaseModel] | None:
    ...
    if not module_name.startswith("content_factory.generation."):
        return None
    try:
