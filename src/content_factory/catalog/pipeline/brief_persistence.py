...
def save_suggestions(
    con: CatalogConnection, brief_id: int, cands: list[SkillCandidate], ev_idmap: dict[str, int]
) -> dict[str, int]:
    ...
    for c in ordered:
        ...
        if stored_decision == "needs_review":
            ...
            severity = (
                "warning"
                if primary_reason in {"novel_skill", "council_split", "fuzzy_match_ambiguous", "low_confidence"}
                else "info"
            )
...
def save_curriculum_plan(
    ...
    try:
        clear_curriculum_plan(con, brief_id, source_policy, commit=False)
        ...
        for row in plan_payload.get("rows", []):
            ...
            """,
                (
                    ...
                    xp,
                    None
                    if row.get("completion_percent") in (None, "")
                    else float(row.get("completion_percent", 0) or 0),
