...
and assembles intake workspace/workflow state.
...
/ ``observability`` / ``curriculum_ops`` / ``intake_reviews`` + stdlib; all pipeline
module aliases are local imports inside the bodies. Owns the intake runtime-state globals.
No back-import of ``app.py`` (module stays acyclic); ``app.py`` re-exports legacy symbols
for the routers.
"""
