...
def functional_program_test(program_name, file_in_text, file_out_text, args):
    with open(out_filename, 'w+') as file_out:
        subprocess.run(f'{program_name} {args}', text=True, stdout=file_out, input=file_in_text,
...
def run_test():
    ...
    try:
        ...
        print('Result for test with number ' + str(test_number) + ': TIMEOUT ERROR 0')
        return
    except UnicodeDecodeError:
        functional_program_test_result = 0
