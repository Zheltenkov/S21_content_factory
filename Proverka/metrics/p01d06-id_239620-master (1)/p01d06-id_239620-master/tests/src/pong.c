...
int collision_detection_left(int *velocity_x, int *velocity_y, int ball_pos_x, int ball_pos_y,
                             int left_rocket_pos_y);
int collision_detection_right(int *velocity_x, int *velocity_y, int ball_pos_x, int ball_pos_y,
                              int right_rocket_pos_y);
...
int main() {
    ...
    char buffer[25][80];
    char *field[25];
    for (int i = 0; i < 25; i++) field[i] = &buffer[i][0];
    ...
    while (1) {
        ...
        scanf("%c", &c);
        if (c == 'a')
            if (left_rocket_pos_y > 1) left_rocket_pos_y--;

        if (c == 'z')
            if (left_rocket_pos_y < 24) left_rocket_pos_y++;

        if (c == 'k')
            if (right_rocket_pos_y > 1) right_rocket_pos_y--;

        if (c == 'm')
            if (right_rocket_pos_y < 24) right_rocket_pos_y++;

        collision_detection_wall(&ball_velocity_x, &ball_velocity_y, ball_pos_x, ball_pos_y);

        if (collision_detection_left(&ball_velocity_x, &ball_velocity_y, ball_pos_x, ball_pos_y,
                                     left_rocket_pos_y)) {
            count_right++;
        ...
        if (collision_detection_right(&ball_velocity_x, &ball_velocity_y, ball_pos_x, ball_pos_y,
                                      right_rocket_pos_y)) {
            count_left++;
            left_rocket_pos_y = 12, right_rocket_pos_y = 12;
            ball_velocity_x = 1, ball_velocity_y = 1, ball_pos_x = 40, ball_pos_y = 12;
...
void add_ball_to_field(char **field, int ball_y_pos, int ball_x_pos) { field[ball_y_pos][ball_x_pos] = '*'; }
...
void collision_detection_wall(int *velocity_x, int *velocity_y, int ball_pos_x, int ball_pos_y) {
    if ((ball_pos_y + *velocity_y) < 2 || (ball_pos_y + *velocity_y) > 23) *velocity_y = -*velocity_y;
}

void draw_field(char **field) {
    for (int i = 0; i < 25; i++) {
        for (int j = 0; j < 80; j++) putchar(field[i][j]);
        putchar('\n');
    }
}
