...
from content_factory.api.routers.download import download_results
from content_factory.api.utils.result_cache import get_result, store_result
from content_factory.generation.orchestrator import OrchestratorResult
...
class TestDownloadAccessControl:
    ...
    async def test_download_own_result_from_cache(self, mock_user, mock_result):
        ...
        # Мокаем зависимости
        with patch("content_factory.api.routers.download.get_result", return_value=get_result(request_id)):
            with patch("content_factory.api.routers.download.get_generation_result", return_value=None):
                with patch("content_factory.api.routers.download.get_report_by_request_id", return_value=mock_result.report_json):
    ...
    async def test_download_other_user_result_from_db(self, mock_user, mock_other_user, mock_db_result):
        ...
        # Мокаем зависимости
        with patch("content_factory.api.routers.download.get_result", return_value=None):
            with patch("content_factory.api.routers.download.get_generation_result", return_value=mock_db_result):
                # Попытка скачать от имени другого пользователя должна вызвать 403
                with pytest.raises(HTTPException) as exc_info:
                    await download_results(
                        request_id=request_id,
