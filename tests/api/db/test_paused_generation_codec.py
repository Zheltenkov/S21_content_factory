...
from content_factory.generation.observability import UnifiedTraceSink
...
def test_paused_generation_codec_hydrates_new_flow_step_type_name() -> None:
    hydrated = hydrate_context(
        {
            "step": {
                "__paused_type__": "content_factory.generation.workflow.flow_runner:FlowExecutionStep",
                "data": {
                    "node_id": "practice",
                    "node_name": "Practice",
                    "status": "paused",
...
def test_paused_generation_codec_records_model_validation_compatibility_event() -> None:
    hydrated = hydrate_context(
        {
            "seed": {
                "__paused_type__": "content_factory.generation.models.schemas:ProjectSeed",
                "data": {"language": "ru"},
            }
        }
    )
    ...
    assert hydrated["compatibility_events"][0]["metadata"] == {
        "type_name": "content_factory.generation.models.schemas:ProjectSeed"
    }
