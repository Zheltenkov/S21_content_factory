...
def test_grounding_accepts_command_output_extracted_by_tshark(workspace_tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("content_factory.audit.artifacts.shutil.which", lambda name: "tshark" if name == "tshark" else None)
    monkeypatch.setattr("content_factory.audit.artifacts.subprocess.run", fake_run)
